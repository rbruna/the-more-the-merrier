...
if strcmpi ( format, 'int16' )
    nums = floor ( ( dend - dbeg ) / filedata.header.nchannels / 2 );
else
    nums = floor ( ( dend - dbeg ) / filedata.header.nchannels / 4 );
end
if send > nums
    send = nums;
end
...
if strcmpi ( format, 'int16' )
    pbeg = ( sbeg - 1 ) * 2 * filedata.header.nchannels;
else
    pbeg = ( sbeg - 1 ) * 4 * filedata.header.nchannels;
end
