...
event = struct ( 'type', {}, 'sample', {}, 'value', {} );
...
% Gets the list of channels to show (without phiçysiological channels).
chanlabel = hdr.label;
...
axes ( 'Tag', 'chandata', 'Position', [ 0.130 0.110 0.775 0.815 ], 'NextPlot', 'add', 'YLim', [ 0 1 ], 'XTick', [], 'YTick', [] );
...
channel = union ( cfg.channel, opt.hdr.label ( cfg.physio ), 'stable' );

% Checks if the list of showed channels have changed.
samechan = isfield ( opt, 'oldchan') && isequal ( opt.oldchan, channel );
opt.oldchan = channel;
...
if isempty ( opt.orgdata )
    data = ft_read_data  ( cfg.datafile, 'header', opt.hdr, 'begsample', begsample, 'endsample', endsample, 'chanindx', chanindx, 'checkboundary', strcmp ( cfg.continuous, 'no' ), 'dataformat', cfg.dataformat, 'headerformat', cfg.headerformat );
else
    data = ft_fetch_data ( opt.orgdata,  'header', opt.hdr, 'begsample', begsample, 'endsample', endsample, 'chanindx', chanindx, 'allowoverlap', true );
end
...
eventtime  = [ event.sample ]' - begsample + offset;
...
for eindex = 1: numel ( event )
    ...
    % Draws the event.
    handle = plot ( ha, eventtime ( [ eindex eindex ] ), [ 0 1 ], 'Color', eventcolor ( eindex, : ), 'Tag', 'event' );
    ...
    % Writes the label.
    text ( double ( eventtime ( eindex ) ), 1 - double ( voffset ), eventlabel { eindex }, 'Parent', ha, 'Tag', 'event', 'Color', eventcolor ( eindex, : ), 'Horizontalalignment', 'left', 'VerticalAlignment', 'top', 'FontUnits', cfg.fontunits, 'FontSize', cfg.fontsize );
...
if opt.plotLabelFlag == 1
    text ( labelpos, centers, opt.hdr.label ( chanindx ), 'Parent', ha, 'Tag', 'chanlabel', 'HorizontalAlignment', 'right', 'FontUnits', cfg.fontunits, 'FontSize', cfg.fontsize );
end
if opt.plotLabelFlag == 2
    text ( labelpos ( 1: 10: end ), centers ( 1: 10: end ), opt.hdr.label ( chanindx ( 1: 10: end ) ), 'Parent', ha, 'Tag', 'chanlabel', 'HorizontalAlignment', 'right', 'FontUnits', cfg.fontunits, 'FontSize', cfg.fontsize );
end
...
% Gets the focus back to the figure.
