...
config.path.figs  = '../../figs_rev2/violins11/';
...
for cindex = 1: numel ( chans )
    ...
    % Goes through each subject.
    for sindex = 1: numel ( subjs )
        
        % Lists the data sets for the current configuration.
        hindex = strcmp ( { data.channel }, chans ( cindex ) ) & strcmp ( { data.subject }, subjs ( sindex ) );
        
        % Initializes the data.
        datum  = data ( find ( hindex, 1 ) );
        
        % Fills the PLV and ciPLV values.
        datum.plv   = mean ( cat ( 5, data ( hindex ).plv   ), 5 );
        datum.ciplv = mean ( cat ( 5, data ( hindex ).ciplv ), 5 );
        
        data11 ( index, 1 ) = datum;
        index  = index + 1;
    end
end
data = data11;
