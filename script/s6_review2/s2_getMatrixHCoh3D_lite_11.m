...
for findex = 1: numel ( files )
    ...
    datum.appr  = { 'Mean HCoh' 'RMS' 'PCA' 'Centroid' 'Average time series' };
...
for bindex = 1: nband
    ...
    text ( 0, 0, sprintf ( 'HCoh^3^D - %s band', config.bandname { bindex } ), 'HorizontalAlign', 'left', 'VerticalAlign', 'bottom', 'FontSize', 8 )
    xlim ( [ +0 +1 ] )
    ylim ( [ -1 +1 ] )
    axis off
...
drawnow
print ( '-dpng', '-r300', sprintf ( '%sViolins - HCoh %s (lite).png', config.path.figs, config.label ) )
% close
...
for bindex = 1: nband
    ...
    text ( 0, 0, sprintf ( 'ciHCoh^3^D - %s band', config.bandname { bindex } ), 'HorizontalAlign', 'left', 'VerticalAlign', 'bottom', 'FontSize', 8 )
    xlim ( [ +0 +1 ] )
    ylim ( [ -1 +1 ] )
    axis off
...
drawnow
print ( '-dpng', '-r300', sprintf ( '%sViolins - ciHCoh %s (lite).png', config.path.figs, config.label ) )
% close
...
drawnow
print ( '-dpng', '-r300', sprintf ( '%sViolins - (ci)HCoh %s (lite).png', config.path.figs, config.label ) )
% close
