...
% Sets the paths.
config.path.conn  = '../../data/connectivity/plv_1D/';
config.path.corr  = '../../stats/correlations/';
config.path.figs  = '../../figs_rev/violins/';
...
config.bands      = { 'Theta' 'Alpha' 'Beta1' };
config.bandname   = { '\theta' '\alpha' 'Low \beta' };
...
% Creates the output folders, if required.
if ~exist ( config.path.figs, 'dir' ), mkdir ( config.path.figs ), end
if ~exist ( config.path.corr, 'dir' ), mkdir ( config.path.corr ), end
...
for findex = 1: numel ( files )
    ...
    for bindex = 1: numel ( band )
        datum.plv   ( :, :, 1, bindex ) = band ( bindex ).plv_avg_all;
        datum.plv   ( :, :, 2, bindex ) = band ( bindex ).plv_rms_all;
        datum.plv   ( :, :, 3, bindex ) = band ( bindex ).plv_avg_pc;
        datum.plv   ( :, :, 4, bindex ) = band ( bindex ).plv_avg_centroid;
        datum.plv   ( :, :, 5, bindex ) = band ( bindex ).plv_avg_mean;
        datum.ciplv ( :, :, 1, bindex ) = band ( bindex ).ciplv_avg_all;
        datum.ciplv ( :, :, 2, bindex ) = band ( bindex ).ciplv_rms_all;
        datum.ciplv ( :, :, 3, bindex ) = band ( bindex ).ciplv_avg_pc;
        datum.ciplv ( :, :, 4, bindex ) = band ( bindex ).ciplv_avg_centroid;
        datum.ciplv ( :, :, 5, bindex ) = band ( bindex ).ciplv_avg_mean;
    end
...
corrdata = [];
corrdata.bandnames   = config.bandname;
corrdata.metrics     = config.metrics;
corrdata.plv_intra   = r_plv   ( n, :, : );
corrdata.ciplv_intra = r_ciplv ( n, :, : );
corrdata.plv_inter   = r_plv   ( m, :, : );
corrdata.ciplv_inter = r_ciplv ( m, :, : );

save ( '-v6', sprintf ( '%splv_%s', config.path.corr, config.label ), '-struct', 'corrdata' )


height = [  5.00  5.00  8.40  0.00 ];
...
for bindex = 1: nband
    ...
    offset = ( nband - bindex ) * [ 0.00 2.70 0.00 0.00 ] + [ 0.00 0.70 0.00 0.00 ];
    
    axes ( 'Units', 'centimeters', 'NextPlot', 'add', 'FontSize', 8 )
    drawnow
    set ( gca, 'Position', [ 0.45 2.00 7.75 0.50 ] + offset )
    
    text ( 0, 0, sprintf ( 'PLV - %s band', config.bandname { bindex } ), 'HorizontalAlign', 'left', 'VerticalAlign', 'bottom', 'FontSize', 8 )
    xlim ( [ +0 +1 ] )
    ylim ( [ -1 +1 ] )
    axis off
    ...
    my_violinplot ( num2cell ( r_plv ( m, :, bindex ), 1 ), [ -1 +1 ] )
    ylim ( [ -1 +1 ] )
    set ( gca, 'YTick', [ -1 0 +1 ] );
    set ( gca, 'XTick', [] );
end
...
drawnow
print ( '-dpng', '-r300', sprintf ( '%sViolins - PLV %s (lite).png', config.path.figs, config.label ) )
% close
...
for bindex = 1: nband
    ...
    text ( 0, 0, sprintf ( 'ciPLV - %s band', config.bandname { bindex } ), 'HorizontalAlign', 'left', 'VerticalAlign', 'bottom', 'FontSize', 8 )
    xlim ( [ +0 +1 ] )
    ylim ( [ -1 +1 ] )
    axis off
    ...
    plot ( [ 0 1 ] * ( napp + 1 ), [ 1 1 ] * r_thres, 'LineStyle', '--', 'Color', [ 0.7 0.7 0.7 ] )
    my_violinplot ( num2cell ( r_ciplv ( m, :, bindex ), 1 ), [ -1 +1 ] )
    ylim ( [ -1 +1 ] )
    set ( gca, 'YTick', [ -1 0 +1 ] );
    set ( gca, 'XTick', [] );
end
...
drawnow
print ( '-dpng', '-r300', sprintf ( '%sViolins - ciPLV %s (lite).png', config.path.figs, config.label ) )
% close


height = [  5.00  5.00 16.80  0.00 ];
height (4) = 2.7 * nband - 0.10 + 0.80;
...
for bindex = 1: nband
    ...
    % Calculates the vertical offset.
    offset = ( nband - bindex ) * [ 0.00 2.70 0.00 0.00 ] + [ 8.45 0.70 0.00 0.00 ];
...
axes ( 'Units', 'centimeters', 'NextPlot', 'add', 'FontSize', 8, 'Visible', 'off' )
drawnow
set ( gca, 'Position', [ 8.90 0.00 7.75 1.00 ] )
...
drawnow
print ( '-dpng', '-r300', sprintf ( '%sViolins - (ci)PLV %s (lite).png', config.path.figs, config.label ) )
% close
