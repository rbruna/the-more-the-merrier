...
% Sets the paths.
config.path.conn  = '../../data/connectivity/hcoh_1D/';
...
save ( '-v6', sprintf ( '%shcoh_%s', config.path.corr, config.label ), '-struct', 'corrdata' )
...
for bindex = 1: nband
    ...
    text ( 0, 0, sprintf ( 'HCoh - %s band', config.bandname { bindex } ), 'HorizontalAlign', 'left', 'VerticalAlign', 'bottom', 'FontSize', 8 )
    xlim ( [ +0 +1 ] )
    ylim ( [ -1 +1 ] )
    axis off
...
for bindex = 1: nband
    ...
    text ( 0, 0, sprintf ( 'ciHCoh - %s band', config.bandname { bindex } ), 'HorizontalAlign', 'left', 'VerticalAlign', 'bottom', 'FontSize', 8 )
    xlim ( [ +0 +1 ] )
    ylim ( [ -1 +1 ] )
    axis off
