...
config.bands (1).edges = [  5.0  7.5 ];
config.bands (1).fs    = 1000;

config.bands (2).name  = 'Beta1';
config.bands (2).order = 1800;
config.bands (2).edges = [ 13.0 19.5 ];
...
for file = 1: numel ( files )
    ...
    bandinfos  = bandinfos ( ismember ( { bandinfos.name }, { config.bands.name } ) );
    ...
    for bindex = 1: numel ( bandinfos )
        ...
        % Updates the band information.
        band.edges   = config.bands ( strcmp ( band.name, { config.bands.name } ) ).edges;
        band.order   = config.bands ( strcmp ( band.name, { config.bands.name } ) ).order;
        band.fs      = config.bands ( strcmp ( band.name, { config.bands.name } ) ).fs;
        ...
        fprintf ( 1, '    Calculating the inter-area PLV.\n' );
