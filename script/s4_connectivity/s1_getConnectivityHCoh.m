...
for file = 1: numel ( files )
    ...
    for bindex = 1: numel ( bandinfos )
        ...
        fprintf ( 1, '    Calculating the all-with-all Hilbert coherence.\n' );
        ...
        for mindex = 1: numel ( maps )
            ...
            chcoh      = complex ( zeros ( nsources, nsources, ntrials, 'single' ) );
            
            % Goes through each trial.
            for tindex = 1: ntrials
                
                % Goes to source space.
                trialcov = map.filter * sensccov ( :, :, tindex ) * map.filter';
                
                % Normalizes the covariance by the diagonal.
                trialcov = trialcov ./ sqrt ( diag ( trialcov ) * diag ( trialcov )' );
                
                % Stores the normalized complex covariance.
                chcoh ( :, :, tindex ) = trialcov;
            end
            
            % Stores the all-with all complex Hibert coherence.
            maps ( mindex ).chcoh  = chcoh;
        end
        
        
        
        fprintf ( 1, '    Calculating the inter-area Hilbert coherence.\n' );
        ...
        for mindex = 1: numel ( maps )
            ...
            if ~config.keeptrials
                map.hcoh_rms   = nanmean ( map.hcoh_rms, 3 );
                map.cihcoh_rms = nanmean ( map.cihcoh_rms, 3 );
                map.hcoh_avg   = nanmean ( map.hcoh_avg, 3 );
                map.cihcoh_avg = nanmean ( map.cihcoh_avg, 3 );
            end
            
            % Stores the Hibert coherence.
            maps ( mindex ).hcoh_avg   = map.hcoh_avg;
            maps ( mindex ).hcoh_rms   = map.hcoh_rms;
            maps ( mindex ).cihcoh_avg = map.cihcoh_avg;
            maps ( mindex ).cihcoh_rms = map.cihcoh_rms;
        end
        ...
        for mindex = 1: numel ( maps )
            ...
            band.( sprintf ( 'hcoh_avg_%s',   map.label ) ) = map.hcoh_avg;
            band.( sprintf ( 'hcoh_rms_%s',   map.label ) ) = map.hcoh_rms;
            band.( sprintf ( 'cihcoh_avg_%s', map.label ) ) = map.cihcoh_avg;
            band.( sprintf ( 'cihcoh_rms_%s', map.label ) ) = map.cihcoh_rms;
        end
