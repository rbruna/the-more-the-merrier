...
for findex = 1: numel ( files )
    ...
    for bindex = 1: numel ( band )
        datum.plv   ( :, :, 1, bindex ) = band ( bindex ).plv_rms_all;
        datum.plv   ( :, :, 2, bindex ) = band ( bindex ).plv_rms_pc999;
        datum.plv   ( :, :, 3, bindex ) = band ( bindex ).plv_rms_pc99;
        datum.plv   ( :, :, 4, bindex ) = band ( bindex ).plv_rms_pc95;
        datum.plv   ( :, :, 5, bindex ) = band ( bindex ).plv_rms_pc;
        datum.ciplv ( :, :, 1, bindex ) = band ( bindex ).ciplv_rms_all;
        datum.ciplv ( :, :, 2, bindex ) = band ( bindex ).ciplv_rms_pc999;
        datum.ciplv ( :, :, 3, bindex ) = band ( bindex ).ciplv_rms_pc99;
        datum.ciplv ( :, :, 4, bindex ) = band ( bindex ).ciplv_rms_pc95;
        datum.ciplv ( :, :, 5, bindex ) = band ( bindex ).ciplv_rms_pc;
    end
...
for bindex = 1: nband
    ...
    plot ( [ 0 1 ] * ( napp + 1 ), [ 1 1 ] * r_thres, 'LineStyle', '--', 'Color', [ 0.7 0.7 0.7 ] )
    my_violinplot ( num2cell ( r_plv ( n, 1: 2, bindex ), 1 ), [ -1 +1 ] )
    ylim ( [ -1 +1 ] )
    set ( gca, 'YTick', [ -1 0 +1 ] );
    set ( gca, 'XTick', [] );
end
...
drawnow
print ( '-dpng', '-r300', sprintf ( '%sViolins - PLV %s PCA.png', config.path.figs, config.label ) )
% close
...
for bindex = 1: nband
    ...
    plot ( [ 0 1 ] * ( napp + 1 ), [ 1 1 ] * r_thres, 'LineStyle', '--', 'Color', [ 0.7 0.7 0.7 ] )
    my_violinplot ( num2cell ( r_ciplv ( n, 1: 2, bindex ), 1 ), [ -1 +1 ] )
    ylim ( [ -1 +1 ] )
    set ( gca, 'YTick', [ -1 0 +1 ] );
    set ( gca, 'XTick', [] );
end
...
drawnow
print ( '-dpng', '-r300', sprintf ( '%sViolins - ciPLV %s PCA.png', config.path.figs, config.label ) )
% close

squeeze ( sum ( p_plv   ( m, :, : ) <= 0.05 ) ) / numel ( m )
squeeze ( sum ( p_plv   ( n, :, : ) <= 0.05 ) ) / numel ( n )
squeeze ( sum ( p_ciplv ( m, :, : ) <= 0.05 ) ) / numel ( m )
squeeze ( sum ( p_ciplv ( n, :, : ) <= 0.05 ) ) / numel ( n )
