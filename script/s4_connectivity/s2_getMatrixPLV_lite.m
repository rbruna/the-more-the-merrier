...
% Sets the paths.
config.path.conn  = '../../data/connectivity/plv_99/';
...
for bindex = 1: nband
    ...
    % Calculates the vertical offset.
    offset = ( nband - bindex ) * [ 0.00 2.70 0.00 0.00 ] + [ 8.45 0.80 0.00 0.00 ];
...
squeeze ( sum ( p_plv   ( m, :, : ) <= 0.05 ) )' / numel ( m )
squeeze ( sum ( p_ciplv ( m, :, : ) <= 0.05 ) )' / numel ( m )
