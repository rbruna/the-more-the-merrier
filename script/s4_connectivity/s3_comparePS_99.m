...
[ ~, p_d ]       = ttest ( data2.plv_inter ( :, : ), data1.plv_inter ( :, : ), 'tail', 'both' );
p_d              = reshape ( p_d, shape ( 2: end ) );
[ ~, p_cid ]     = ttest ( data2.ciplv_inter ( :, : ), data1.ciplv_inter ( :, : ), 'tail', 'both' );
...
fprintf ( 1, '%s\n', config.label1 );
fprintf ( 1, '%.8f %.8f %.8f %.8f\n', min ( p_lab1 ( [ 1 2 3 5 ], : ) * 20, 1 ) )
fprintf ( 1, '(ci) %s\n', config.label1 );
fprintf ( 1, '%.8f %.8f %.8f %.8f\n', min ( p_cilab1 ( [ 1 2 3 5 ], : ) * 20, 1 ) )
fprintf ( 1, '\n' );
