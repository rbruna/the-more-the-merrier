...
if ispc, [ state, out ]  = system ( sprintf ( '"%s\\%s" "%s.off" "%s.off"', fileparts ( mfilename ( 'fullpath' ) ), 'meshfix-win.exe', basename, basename ) );
else     [ state, out ]  = system ( sprintf ( '"%s%s" "%s.off" -o "%s" -q -a 0.01', mcpath ( 'meshfix' ), fallbackexeext ( getexeext, 'meshfix' ), basename, basename ) );
end
